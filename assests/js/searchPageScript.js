...
const renderSearchResultHotelCardResults = (prodcutsFromJSONFile) => {
  // JSON file data
  const hotels = prodcutsFromJSONFile;
  //for render cards from json file
  // hotels.filter((hotel)=> hotel.includes('alex'))
  for (let hotelIndex in hotels) {
    resultNumbers ? (resultNumbers.textContent = +hotelIndex + 1) : null;
    let hotelObject = new HotelModel(
      hotels[hotelIndex].id,
      hotels[hotelIndex].order,
      hotels[hotelIndex].url,
      hotels[hotelIndex].name,
      hotels[hotelIndex].type,
      hotels[hotelIndex].description,
      hotels[hotelIndex].stars,
      hotels[hotelIndex].price,
      hotels[hotelIndex].rating,
      hotels[hotelIndex].reviews,
      hotels[hotelIndex].breakfast,
      hotels[hotelIndex].checkInFrom,
      hotels[hotelIndex].checkInTo,
      hotels[hotelIndex].location,
      hotels[hotelIndex].address,
      hotels[hotelIndex].image,
      hotels[hotelIndex].rooms,
      hotels[hotelIndex].images,
      hotels[hotelIndex].categoryReviews,
      hotels[hotelIndex].userReviews
    );
    searchResultHotelCard(hotelObject);
  }
};

renderSearchResultHotelCardResults(IndexedResults);
